      ...
      <head>
        <script
          dangerouslySetInnerHTML={{
            __html:
              '!function(){try{var d=document.documentElement,h=document.head||d;var m=document.querySelector(\'meta[name="viewport"]\');if(!m){m=document.createElement("meta");m.setAttribute("name","viewport");h.insertBefore(m,h.firstChild);}function cssW(){var sw=screen.width||0,dpr=window.devicePixelRatio||1;return(sw>540&&dpr>1)?Math.round(sw/dpr):sw;}function isPhone(){var w=cssW();if(w>=280&&w<=540)return true;if(window.matchMedia&&window.matchMedia("(max-device-width: 512px)").matches)return true;var tp=0;try{tp=navigator.maxTouchPoints||0}catch(e){}if(tp>0)return true;if(window.matchMedia&&window.matchMedia("(pointer: coarse)").matches)return true;return false;}function lock(){var w=cssW();var c=(w>=280&&w<=540)?("width="+w+",initial-scale=1,viewport-fit=cover"):"width=device-width,initial-scale=1,viewport-fit=cover";if(m.getAttribute("content")!==c){m.setAttribute("content",c);if(m.parentNode)m.parentNode.appendChild(m);}}function apply(){lock();var desktop=!isPhone()&&!!(window.matchMedia&&window.matchMedia("(min-width: 1024px) and (hover: hover) and (pointer: fine)").matches);var n=!desktop;d.classList.toggle("is-narrow",n);if(n){d.style.overflowX="hidden";d.style.maxWidth="100%";d.style.width="100%";}else{d.style.removeProperty("overflow-x");d.style.removeProperty("max-width");d.style.removeProperty("width");}}apply();window.addEventListener("resize",apply);window.visualViewport&&window.visualViewport.addEventListener("resize",apply);setTimeout(apply,0);setTimeout(apply,50);setTimeout(apply,200);new MutationObserver(lock).observe(m,{attributes:true,attributeFilter:["content"]});var last={h:"",t:0};function go(a,e){if(!a||e.metaKey||e.ctrlKey||e.shiftKey||e.altKey)return;if(typeof e.button==="number"&&e.button>0)return;var href=a.href;if(!href)return;var now=Date.now();if(last.h===href&&now-last.t<500){e.preventDefault();e.stopImmediatePropagation();return;}last={h:href,t:now};e.preventDefault();e.stopImmediatePropagation();location.assign(href);}function onEvt(e){var t=e.target;if(!t||!t.closest)return;var a=t.closest("a[data-hard-nav]");if(!a)return;go(a,e);}document.addEventListener("click",onEvt,true);document.addEventListener("pointerup",onEvt,true);}catch(e){}}();',
          }}
        />
