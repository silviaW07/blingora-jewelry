  ...
  return campaigns.map((item) => {
    ...
    const amount =
      item.discountAmount != null
        ? `US$ ${toUsdPrice(item.discountAmount.toNumber()).toFixed(0)}`
...
export const getProductDetail = withResult(
  async (input: GetProductDetailInput): Promise<GetProductDetailOutput> => {
    ...
    const skus: ProductSkuData[] = product.skus.map((sku) => {
      const attrs = (sku.attributeJson as object) as SkuAttribute[]
      ...
      const originalPriceRmb =
        cost !== null && cost > 0
          ...
          : sku.originalPrice
            ...
            : null
      const price = toUsdPrice(priceRmb)
      return {
        id: sku.id,
        skuCode: sku.skuCode,
    ...
    const detailData: ProductDetailData = {
      ...
      priceMax,
      minOrderQty,
      priceTiers: buildPriceTiers(prices, minOrderQty),
      coupons: mapActiveCoupons(couponCampaigns),
    }
