...
const parseCategoryTopPromotion = (rawContent: unknown, fallbackActive: boolean): CategoryTopPromotionConfig | null => {
  ...
  return {
    ...
    font_size: (() => {
      ...
      if (fontRaw === 'sm' || fontRaw === 'md' || fontRaw === 'lg') return fontRaw
...
export const getCategoryList = withResult(async (input?: GetCategoryListInput): Promise<GetCategoryListOutput> => {
  ...
  return {
    list: mainCategories.map(cat => ({
      category_id: cat.id,
      category_name: resolveCategoryDisplayName(
        cat.translationsJson,
        cat.navConfig?.navTitle?.trim() || cat.name,
        lang,
      ),
...
export const getCategoryDetail = withResult(async (input: GetCategoryDetailInput): Promise<GetCategoryDetailOutput> => {
  ...
  // Orphan / parent-missing L2 (common for Brand SIDE_NAV brands): treat self as main
  // so storefront Brand list items remain navigable and titled correctly.
  ...
  let items: ProductItem[] = dbProducts
    ...
    return {
      ...
      brand_category_name: p.brandCategory
        ? resolveCategoryDisplayName(p.brandCategory.translationsJson, p.brandCategory.name, lang)
        : null,
