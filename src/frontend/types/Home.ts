...
const toUsdPrice = (rmbPrice: number | null | undefined): number => {
  ...
  return Math.round((rmbPrice / USD_EXCHANGE_RATE) * 100) / 100
}
  ...
  if (categoryZoneCategoryIds.length > 0) {
    ...
    const mapLatestProductCard = (
      ...
      return {
        ...
        price: toUsdPrice(priceRmb),
        originalPrice: originalPriceRmb !== null ? toUsdPrice(originalPriceRmb) : null,
            ...
            priceRmb === null
              ...
              : cost !== null && cost > 0
                ...
                : defaultSku?.originalPrice
                  ? defaultSku.originalPrice.toNumber()
                  : null
          const price = priceRmb !== null ? toUsdPrice(priceRmb) : null
          const originalPrice = originalPriceRmb !== null ? toUsdPrice(originalPriceRmb) : null
          const translatedName = resolveProductDisplayName(
            product.name,
...
export const getDailyNewArrivalProducts = withResult(async (
  ...
  const list = dbProducts.map((product) => mapActiveProductToItem(product, input.lang))
