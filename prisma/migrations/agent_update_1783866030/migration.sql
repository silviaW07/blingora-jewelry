...
-- AlterTable
ALTER TABLE `keywordgroup` ADD COLUMN `floorIcon` VARCHAR(160) NULL,
    ADD COLUMN `floorLink` VARCHAR(255) NULL,
    ADD COLUMN `floorTitle` VARCHAR(160) NULL,
    ADD COLUMN `homepageSortWeight` INTEGER NOT NULL DEFAULT 0,
    ADD COLUMN `showOnHomepage` BOOLEAN NOT NULL DEFAULT false;

-- CreateIndex
CREATE INDEX `keywordgroup_showOnHomepage_idx` ON `keywordgroup`(`showOnHomepage`);

-- CreateIndex
CREATE INDEX `keywordgroup_homepageSortWeight_idx` ON `keywordgroup`(`homepageSortWeight`);
