...
-- AlterTable
ALTER TABLE `category` ADD COLUMN `brandCode` VARCHAR(80) NULL,
    ADD COLUMN `brandKeywordsJson` JSON NULL,
    ADD COLUMN `homepageConfigJson` JSON NULL,
    ADD COLUMN `isBrandCategory` BOOLEAN NOT NULL DEFAULT false;

-- AlterTable
ALTER TABLE `product` ADD COLUMN `autoBrandMatched` BOOLEAN NOT NULL DEFAULT false,
    ADD COLUMN `brandCategoryId` VARCHAR(36) NULL,
    ADD COLUMN `brandMatchKeyword` VARCHAR(120) NULL;

-- AlterTable
ALTER TABLE `sitesetting` MODIFY `settingType` ENUM('PROMO_BAR', 'HERO_BANNER', 'HOT_MATERIAL', 'LOOKBOOK', 'TRUST_BADGE', 'HOT_SEARCH', 'FOOTER_LINK', 'FLOAT_CONTACT', 'HOMEPAGE_POSTER', 'HOME_SECTION', 'STATIC_COPY', 'EMAIL_TEMPLATE', 'PAYMENT_METHOD', 'CURRENCY_SETTING', 'EXCHANGE_RATE', 'SHIPPING_TEMPLATE', 'TAX_RULE', 'ROLE_PERMISSION', 'HOME_BRAND_SECTION', 'HOME_REVIEW_SECTION') NOT NULL;

-- CreateIndex
CREATE INDEX `category_isBrandCategory_idx` ON `category`(`isBrandCategory`);

-- CreateIndex
CREATE INDEX `category_brandCode_idx` ON `category`(`brandCode`);

-- CreateIndex
CREATE INDEX `product_brandCategoryId_idx` ON `product`(`brandCategoryId`);
...
-- AddForeignKey
ALTER TABLE `product` ADD CONSTRAINT `product_brandCategoryId_fkey` FOREIGN KEY (`brandCategoryId`) REFERENCES `category`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
